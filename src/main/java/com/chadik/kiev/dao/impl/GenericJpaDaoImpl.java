 ...
 */
@Repository
public abstract class GenericJpaDaoImpl<T, ID extends Serializable> implements IGenericJpaDao<T, ID> {
    ...
    public List findAll() {
        Session hibernateSession = getSession();
        List T = null;
        Query query = hibernateSession.createQuery("from " + getClazz().getName());
        T = query.list();
        return T;
    }
